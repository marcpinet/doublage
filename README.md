...
5. Optional **voice FX**: the FX button (Live and Pro) opens a menu of effects baked into your takes — deep/high pitch, echo (cavern), "in your head" inner voice, power aura, robot, autotune. Test the result live with the "Hear myself" mic monitor before recording (headphones!). You can switch or turn the effect off **mid-take** (deep voice for the possession scene, normal after) — each change applies from that moment. Takes are recorded dry and processed right after you stop, so a glitch can never ruin a take, and what you replay is exactly what the host receives (takes get an FX badge on both sides).
...
- Python 3.10+ then:
  ...
  ```
- **Windows only**: `requirements.txt` also installs `triton-windows` (the GPU kernels Whisper uses; skipped on Linux/macOS). Triton compiles its kernels at runtime with MSVC, so install the [Visual Studio Build Tools](https://visualstudio.microsoft.com/visual-studio-build-tools/) with the **"Desktop development with C++"** workload (any Visual Studio 2022 edition works, the free Build Tools are enough). Without it everything still runs except those Whisper GPU kernels. `run.bat` (below) sets the MSVC environment up for you.
...
On Windows you can double-click **`run.bat`** instead: it relaunches itself elevated (UAC), loads the MSVC environment for triton (auto-detected with `vswhere`, so any Visual Studio / Build Tools edition and install path works — or point the `VCVARS` env var at a specific `vcvars64.bat`), then starts the app from `.venv`. If MSVC is missing it warns and launches anyway.
...
It is configured via the `.env` at the root (see `.env.example`): `HOST`, `PORT`, `STORAGE_DIR`, `ROOM_TTL_MIN`, `EMPTY_ROOM_GRACE_MIN`, `MAX_ROOM_MB`, `MAX_TAKE_MB`, `MAX_ROOMS`, `MAX_MEMBERS_PER_ROOM`, `ALLOWED_ORIGINS`, and `PUBLIC_URL` (used for the Open Graph link preview). The defaults are fine (port 21826, ephemeral rooms purged after inactivity and shortly after everyone has left, size caps and per-IP rate limits against abuse).
...
| `dubbing/web/fx/` | Voice FX engine: YIN + TD-PSOLA pitch (AudioWorklet for the mic monitor, Worker for offline take rendering) + `dsp.mjs` shared DSP core |
...
| `scripts/test_fx_dsp.mjs` | Objective regression test of the FX pitch engine (`node scripts/test_fx_dsp.mjs`, ±5 cents) |
